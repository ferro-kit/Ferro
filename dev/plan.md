...
### 帮助页精简：其余 22 页（2026-08-26 提出）

`dataset` 三页已按模板精简（collect 117→32、filter 117→43、merge 63→35）。
模板是：一句话用途 + **完整参数表** + 输出布局 + 2~3 个例子 + 一行
`Full documentation:  ferro doc <topic>`。搬走的是「为什么这样」的段落，
留下的只有**改变命令行读法**的判据（阈值 0 = 关闭、区间计存活帧）。

上限定的是 40 行，`filter` 实际 43 —— 参数表本身 18 行，是唯一必须完整的
一段，超出的三行由它而来。**不为了压进 40 行去砍参数表**。

剩下 22 页照此办理之前，先看 `dataset` 三页的实物。判据在实施中没有被推翻，
但形式是否合用只有用过才知道。

顺带：精简这三页时把手册对应页补全（`filter.md` 196→230、`collect.md`
161→218），并修掉手册自己的漂移 —— `filter.md` 的漏斗图缺两条几何判据、
`filter.md` 与 `collect.md` 都把已实现的 `merge` 写成「未实现」。

### `ferro doc` 的表格渲染（2026-08-26 提出）

现在原样打 markdown，零依赖。表格用 `|---|` 紧凑分隔符，终端里不好看，
`cli-reference.md` 有 266 行表格。两条路：

- 极简重排：按列宽重排 `|` 分隔的行，几十行代码，不引依赖
- `termimad` 之类：好看，但多一个依赖

**先用着，不好看再说**。这是「做出来看实物再定」的一项，不是设计未定。
