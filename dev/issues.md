...
| 位置 | 陷阱 | 正确做法 |
|---|---|---|
| 需求的两处表述冲突时 | 以文档/脚本注释为准 | 以**用户的直接表述**为准。这里踩过：脚本注释写「每个来源独立切 set」，用户口述是「每个 system 一个 set」，按前者实现就错了。注释描述的是当时的实现，口述描述的是意图 |
...
| by-source 的语义 | 「每个来源独立切 set」 | 是**一个 system 一个 set**，`--set-size` 不参与。两种读法都能自圆其说（前者也保证 set 不横跨条件），但只有后者让「选一个 set 当验证集」等价于「留出一个完整条件」。writer 因此需要一个显式边界的入口 —— 均匀切表达不了这件事 |
