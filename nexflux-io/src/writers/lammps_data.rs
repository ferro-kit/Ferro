...
pub fn write_lammps_data(trajectory: &Trajectory, path: &str) -> Result<()> {
    ...
    let comment = trajectory.metadata.source.as_deref().unwrap_or("LAMMPS data file written by nexflux");
    writeln!(w, "{comment}")?;
    writeln!(w)?;
...
fn cell_to_lammps(cell: &nexflux_core::Cell) -> (f64, f64, f64, f64, f64, f64) {
...
fn lammps_cell_matrix(lx: f64, ly: f64, lz: f64, xy: f64, xz: f64, yz: f64) -> nexflux_core::Cell {
    use nalgebra::Matrix3;
    nexflux_core::Cell::from_matrix(Matrix3::new(
        lx,  0.0, 0.0,
        xy,  ly,  0.0,
        xz,  yz,  lz,
    ))
...
fn bounding_box(frame: &nexflux_core::Frame) -> (f64, f64, f64, f64, f64, f64) {
