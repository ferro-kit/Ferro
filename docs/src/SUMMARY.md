# Summary

[Introduction](introduction.md)
[Installation](installation.md)

---

# Core Data Model

- [Types: Atom, Frame, Trajectory, Cell](data-model.md)

# Analysis Methods

- [Radial Distribution Function g(r)](analysis/gr.md)
- [Structure Factor S(q)](analysis/sq.md)
- [Mean Squared Displacement (MSD)](analysis/msd.md)
- [Bond Angle Distribution](analysis/angle.md)
- [Van Hove Self-Correlation Gs(r,τ)](analysis/vanhove.md)
- [Velocity Autocorrelation (VACF)](analysis/vacf.md)
- [Rotational Correlation C₂(t)](analysis/rotcorr.md)
- [3-D Spatial Density Maps](analysis/cube-density.md)
- [Jump Distance Distribution](analysis/cube-jump.md)
- [Hard-Sphere Occupancy](analysis/cube-radius.md)
- [Cluster SDF](analysis/cube-sdf.md)
- [Averaged Charge-Density SDF](analysis/chg-sdf.md)
- [Glass Network Analysis](analysis/network.md)

# Machine-Learning Datasets

- [Collecting AIMD Output](dataset/collect.md)

# Input Generation

- [Job Builders](workflow/job-builders.md)
- [Spin / Unpaired-Electron Estimation](workflow/spin.md)

# Python API

- [Python Bindings](python.md)

# Reference

- [CLI Reference](cli-reference.md)
